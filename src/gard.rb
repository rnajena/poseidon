...
class Gard
  ...
  def run_gard(aln, nuc_bias, rate_variation, rate_classes, output, threads, parameter_string)
    ...
    unless File.exists?("#{File.dirname(output)}/gard.log")
      ...
      gard_log << `(echo "#{aln}"; echo "#{aln}"; echo "#{nuc_bias}"; echo "#{rate_variation}"; echo "#{rate_classes}"; echo "#{output}") | #{OPENMPI} -np #{threads} #{HYPHY} #{GARD_TEMPLATE_BATCH}`
      gard_log.close
  ...
  def run_gard_processor(output, threads, parameter_string)
    ...
    unless File.exists?("#{File.dirname(output)}/gard_processor.log")
      ...
      gard_processor_log << `(echo "#{gard_result_file}"; echo "#{gard_splits_file}") | #{OPENMPI} -np #{threads} #{HYPHY} #{GARD_PROCESSOR_TEMPLATE_BATCH}`
